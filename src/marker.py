...
class MarkerDetector:
    def __init__(
        ...
        target_marker_width: float = 0.10,
        nav_marker_width: float = 0.27,
    ) -> None:
    ...
    def process_frame(self, frame: np.ndarray):
        ...
        target_info = self.estimate_pose_and_draw_axes(
            frame, target_corners, target_ids, "Target"
        )
        nav_info = self.estimate_pose_and_draw_axes(
            frame, nav_corners, nav_ids, "Navigation"
        )
        return frame, {"Target": target_info, "Navigation": nav_info}

    def draw_markers(
        self, frame: np.ndarray, corners: np.ndarray, ids: list, marker_class: str = ""
    ):
    ...
    def estimate_pose_and_draw_axes(
        self, frame: np.ndarray, corners: np.array, ids: list, marker_type: str
    ):
        if ids is None:
            return None
        ...
        marker_width = (
            self.target_marker_width
            if marker_type == "Target"
            else self.nav_marker_width
        )
        ...
        for i, (corner, rot_vec, tran_vec) in enumerate(
            zip(corners, rotations_vecs, translation_vecs)
        ):
            aruco_id = ids[i][0]
